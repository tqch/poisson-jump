import math
import torch
import torch.nn as nn
try:
    from .modules import Linear, Conv2d, SamePad2d, Sequential, pair
    from .functions import get_timestep_embedding
except ImportError:
    import sys
    from pathlib import Path
    sys.path.append(str(Path(__file__).resolve().parents[2]))
    from poisson_jump.nets.modules import Linear, Conv2d, SamePad2d, Sequential, pair
    from poisson_jump.nets.functions import get_timestep_embedding

DEFAULT_NONLINEARITY = nn.SiLU()  # f(x)=x*sigmoid(x)


class DEFAULT_NORMALIZER(nn.GroupNorm):
    def __init__(self, num_channels, num_groups=32):
        super().__init__(num_groups=num_groups, num_channels=num_channels)


class AttentionBlock(nn.Module):
    normalize = DEFAULT_NORMALIZER

    def __init__(
            self,
            in_channels,
            mid_channels=None,
            out_channels=None
    ):
        super(AttentionBlock, self).__init__()
        mid_channels = mid_channels or in_channels
        out_channels = out_channels or in_channels
        self.norm = self.normalize(in_channels)
        self.project_in = Conv2d(in_channels, 3 * mid_channels, 1)
        self.project_out = Conv2d(mid_channels, out_channels, 1, init_scale=0.)
        self.in_channels = in_channels
        self.mid_channels = mid_channels
        self.out_channels = out_channels
        self.skip = nn.Identity() if in_channels == out_channels else Conv2d(in_channels, out_channels, 1)

    @staticmethod
    def qkv(q, k, v):
        B, C, H, W = q.shape
        w = torch.einsum("bchw, bcHW -> bhwHW", q, k)
        w = torch.softmax(
            w.reshape(B, H, W, H * W) / math.sqrt(C), dim=-1).reshape(B, H, W, H, W)
        out = torch.einsum("bhwHW, bcHW -> bchw", w, v)
        return out

    def forward(self, x, **kwargs):
        skip = self.skip(x)
        C = x.shape[1]
        assert C == self.in_channels
        q, k, v = self.project_in(self.norm(x)).chunk(3, dim=1)
        x = self.qkv(q, k, v)
        x = self.project_out(x)
        return x + skip


class ResidualBlock(nn.Module):
    normalize = DEFAULT_NORMALIZER
    nonlinearity = DEFAULT_NONLINEARITY

    def __init__(
            self,
            in_channels,
            out_channels,
            embed_dim,
            resample_type="none",
            drop_rate=0.,
            temp_cond=True,
            scale_shift=True
    ):
        super(ResidualBlock, self).__init__()

        self.norm1 = self.normalize(in_channels)
        self.conv1 = Conv2d(in_channels, out_channels, 3, 1, 1)
        self.temp_cond = temp_cond
        self.scale_shift = scale_shift
        if temp_cond:
            self.fc = Linear(embed_dim, out_channels * (2 if scale_shift else 1))
        self.norm2 = self.normalize(out_channels)
        self.conv2 = Conv2d(out_channels, out_channels, 3, 1, 1, init_scale=0.)
        self.skip = nn.Identity() if in_channels == out_channels else Conv2d(in_channels, out_channels, 1)
        self.dropout = nn.Dropout(p=drop_rate, inplace=True)

        self.resample = self.get_resample(resample_type)

    @staticmethod
    def get_resample(resample_type):
        if resample_type == "down":
            return nn.AvgPool2d((2, 2))
        elif resample_type == "up":
            return nn.Upsample(scale_factor=2, mode="nearest")
        else:
            return nn.Identity()

    def forward(self, x, t_emb=None):
        skip = self.skip(self.resample(x))
        x = self.conv1(self.resample(self.nonlinearity(self.norm1(x))))
        if self.temp_cond and t_emb is not None:
            t_emb = self.fc(self.nonlinearity(t_emb))[:, :, None, None]
            if self.scale_shift:
                scale, shift = t_emb.chunk(2, dim=1)
                x = (1 + scale) * self.norm2(x) + shift
            else:
                x += t_emb
                x = self.norm2(x)
        else:
            x = self.norm2(x)
        x = self.conv2(self.dropout(self.nonlinearity(x)))
        return x + skip


class UNet(nn.Module):
    normalize = DEFAULT_NORMALIZER
    nonlinearity = DEFAULT_NONLINEARITY

    def __init__(
            self,
            in_channels,
            hid_channels,
            out_channels,
            ch_multipliers,
            num_res_blocks,
            apply_attn,
            time_embedding_dim=None,
            drop_rate=0.,
            resample_with_conv=True,
            resample_with_res=False,
            temp_cond=True,  # use temporal conditioning
            scale_shift=False,
            continuous_t=False
    ):
        super(UNet, self).__init__()
        self.in_channels = in_channels
        self.hid_channels = hid_channels
        self.out_channels = out_channels
        self.time_embedding_dim = time_embedding_dim or 4 * self.hid_channels
        levels = len(ch_multipliers)
        self.ch_multipliers = ch_multipliers
        if isinstance(apply_attn, bool):
            apply_attn = [apply_attn for _ in range(levels)]
        self.apply_attn = apply_attn
        self.num_res_blocks = num_res_blocks
        self.drop_rate = drop_rate
        self.resample_with_conv = resample_with_conv
        self.resample_with_res = resample_with_res

        assert not (resample_with_conv and resample_with_res)

        self.temp_cond = temp_cond
        self.scale_shift = scale_shift
        self.continuous_t = continuous_t

        if self.temp_cond:
            self.embed = Sequential(
                Linear(self.hid_channels, self.time_embedding_dim),
                self.nonlinearity,
                Linear(self.time_embedding_dim, self.time_embedding_dim))

        self.in_conv = Conv2d(in_channels, hid_channels, 3, 1, 1)
        self.levels = levels
        self.downsamples = nn.ModuleDict({f"level_{i}": self.downsample_level(i) for i in range(levels)})
        mid_channels = ch_multipliers[-1] * hid_channels
        self.middle = Sequential(
            self.resid_block(mid_channels, mid_channels),
            AttentionBlock(mid_channels),
            self.resid_block(mid_channels, mid_channels))

        self.upsamples = nn.ModuleDict({f"level_{i}": self.upsample_level(i) for i in range(levels)})
        self.out_conv = nn.Sequential(
            self.normalize(hid_channels * ch_multipliers[0]),
            self.nonlinearity,
            Conv2d(hid_channels * ch_multipliers[0], out_channels, 3, 1, 1, init_scale=0.))

    def resid_block(self, in_chans, out_chans, resample_type="none"):
        return ResidualBlock(
            in_chans, out_chans,
            embed_dim=self.time_embedding_dim,
            resample_type=resample_type,
            drop_rate=self.drop_rate,
            temp_cond=self.temp_cond,
            scale_shift=self.scale_shift
        )

    def get_level_block(self, level):
        if self.apply_attn[level]:
            def block(in_chans, out_chans):
                return Sequential(
                    self.resid_block(in_chans, out_chans),
                    AttentionBlock(out_chans))
        else:
            def block(in_chans, out_chans):
                return self.resid_block(in_chans, out_chans)
        return block

    def downsample_level(self, level):
        block = self.get_level_block(level)
        prev_chans = self.hid_channels if level == 0 else self.ch_multipliers[level - 1] * self.hid_channels
        curr_chans = self.ch_multipliers[level] * self.hid_channels
        modules = nn.ModuleList([block(prev_chans, curr_chans)])
        for _ in range(self.num_res_blocks - 1):
            modules.append(block(curr_chans, curr_chans))
        if level != self.levels - 1:
            if self.resample_with_res:
                modules.append(self.resid_block(curr_chans, curr_chans, resample_type="down"))
            else:
                if self.resample_with_conv:
                    downsample = Sequential(
                        SamePad2d(3, 2),
                        Conv2d(curr_chans, curr_chans, 3, 2))
                else:
                    downsample = nn.AvgPool2d(2)
                modules.append(downsample)
        return modules

    def upsample_level(self, level):
        block = self.get_level_block(level)
        ch = self.hid_channels
        chs = list(map(lambda x: ch * x, self.ch_multipliers))
        next_chans = ch if level == 0 else chs[level - 1]
        prev_chans = chs[-1] if level == self.levels - 1 else chs[level + 1]
        curr_chans = chs[level]
        modules = nn.ModuleList([block(prev_chans + curr_chans, curr_chans)])
        for _ in range(self.num_res_blocks - 1):
            modules.append(block(2 * curr_chans, curr_chans))
        modules.append(block(next_chans + curr_chans, curr_chans))
        if level != 0:
            if self.resample_with_res:
                modules.append(self.resid_block(curr_chans, curr_chans, resample_type="up"))
            else:
                upsample = [nn.Upsample(scale_factor=2, mode="nearest")]
                if self.resample_with_conv:
                    upsample.append(Conv2d(curr_chans, curr_chans, 3, 1, 1))
                modules.append(Sequential(*upsample))
        return modules

    def forward(self, x, t=None):
        if t is not None:
            if self.continuous_t:
                t = 1000 * t
            t_emb = get_timestep_embedding(t, self.hid_channels)
            t_emb = self.embed(t_emb)
        else:
            t_emb = None

        # downsample
        hs = [self.in_conv(x)]
        for i in range(self.levels):
            downsample = self.downsamples[f"level_{i}"]
            for j, layer in enumerate(downsample):  # noqa
                h = hs[-1]
                if j != self.num_res_blocks:
                    hs.append(layer(h, t_emb=t_emb))
                else:
                    if isinstance(layer, ResidualBlock):
                        hs.append(layer(h, t_emb=t_emb))
                    else:
                        hs.append(layer(h))

        # middle
        h = self.middle(hs[-1], t_emb=t_emb)

        # upsample
        for i in range(self.levels - 1, -1, -1):
            upsample = self.upsamples[f"level_{i}"]
            for j, layer in enumerate(upsample):  # noqa
                if j != self.num_res_blocks + 1:
                    h = layer(torch.cat([h, hs.pop()], dim=1), t_emb=t_emb)
                else:
                    if isinstance(layer, ResidualBlock):
                        h = layer(h, t_emb=t_emb)
                    else:
                        h = layer(h)

        h = self.out_conv(h)
        return h


if __name__ == "__main__":
    # temporally-conditional

    print_model = False

    model = UNet(3, 128, 3, (1, 1, 1), 3, (False, True, True), resample_with_conv=False, resample_with_res=True)
    if print_model:
        print(model)
    x = torch.randn(16, 3, 32, 32)
    out_1 = model(x, t=torch.randint(1000, size=(16,)))
    nn.MSELoss(reduction="mean")(x, out_1).backward()
    torch.optim.Adam(model.parameters(), lr=2e-4).step()
    model.zero_grad(set_to_none=True)
    out_2 = model(x, t=None)  # disable condition
    print(torch.allclose(out_1, out_2))

    # unconditional
    model = UNet(3, 64, 3, (1, 2, 4), 2, (False, True, True), temp_cond=False)
    if print_model:
        print(model)
    out = model(torch.randn(16, 3, 32, 32))
    print(out.shape)
