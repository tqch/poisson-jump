...
def DEFAULT_INITIALIZER(x, scale: float = 1.):
    """
...
class _Conv(nn.Module):
    def __init__(
        ...
        self.kernel_size = self.parse(kernel_size)
        self.groups = groups
        ...
        if bias:
            ...
            self.register_parameter("bias", None)
        self.kernel_size = self.parse(kernel_size)
        self.stride = self.parse(stride)
        self.padding = self.parse(padding)
        self.dilation = self.parse(dilation)
        self.groups = groups
